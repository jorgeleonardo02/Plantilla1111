...
export class EncabezadoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const socket = new SockJS('http://localhost:8888/pantillaApp/websocket');
    ...
    this.stompClient.onConnect = (frame) => {
      ...
      this.stompSubscription = this.stompClient.subscribe('/topic/actualizacion-contenidos', (message: Message) => {
        const nuevosContenidos: Contenido[] = JSON.parse(message.body);
        console.log("nuevosContenidos:", nuevosContenidos);
        this.actualizarContenidos(nuevosContenidos);
        this.eliminarElementosNoPresentesEnContenidos();
      });
    };
  ...
  calcularSumaTotal(): void {
    this.sumaTotal = 0;
    this.contenidoEnCarrito.forEach(contenido => {
      this.sumaTotal += contenido.precio * (1 + contenido.porcentajeAdmin);
      console.log(this.sumaTotal);
    });
  }
  ...
  // Ventana modal de formulario para llenar la tabla contenido
  public abrirVentanaModalForm(): void {
    const referenciaVentanaModal = this.ventanaModal.open(FormContenidoComponent, {
      width: '60%',
      height: 'auto',
      position: { left: '20%', top: '60px' },
    });
  ...
  actualizarContenidos(nuevosContenidos: Contenido[]): void {
    this.contenidoEnCarrito = nuevosContenidos;
  }

  eliminarElementosNoPresentesEnContenidos(): void {
    const idsContenidos = this.contenidoEnCarrito.map(contenido => contenido.id);
    const elementosNoPresentes = this.contenidoEnCarrito.filter(contenido => !idsContenidos.includes(contenido.id));
    elementosNoPresentes.forEach(elemento => {
      this.carritoService.eliminarElementoDelCarrito(elemento.id);
    });
  }
} */